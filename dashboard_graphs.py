...
class MaternalHealthDashboard:
    ...
    def create_bubble_chart(self):
        ...
        fig = px.scatter(
            df,
            x="State/UT",
            y="Achievement during April to December - Institutional Deliveries - No. Reported - (C)",
            size="Achievement during April to December - % Institutional reported to Estimated - (J=(C/I)*100)",
            color="State/UT",
            hover_name="State/UT",
            labels={
                "State/UT": "Need Assessed",
                "Achievement during April to December - Institutional Deliveries - No. Reported - (C)": "Achievement",
                "Achievement during April to December - % Institutional reported to Estimated - (J=(C/I)*100)": "%Achievement",
            },
        )
        #showing the graph
        st.plotly_chart(fig)
    ...
    def create_pie_chart(self):
        ...
        fig = px.pie(
            df,
            names="State/UT",
            values="Achievement during April to December - Institutional Deliveries - No. Reported - (C)",
            labels={"Achievement during April to December - Institutional Deliveries - No. Reported - (C)": "Institutional Deliveries"}
        )
        st.plotly_chart(fig)
...
1: X-axis (horizontal axis): State/UT
...
2: Y-axis (vertical axis): Achievement during April to December - Institutional Deliveries - No. Reported - (C)
...
if __name__ == "__main__":
    api_key = "579b464db66ec23bdd0000017c671d378dd44b7076f9019f75f8cd30"
    api_endpoint = api_endpoint= f"https://api.data.gov.in/resource/c6f76dc2-1dac-4d4a-8cc3-db2c7da4579e?api-key={api_key}&format=csv"
